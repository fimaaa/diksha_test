...
object DateUtils {
    ...
    fun getDateStringWithFormat(
        ...
        return try {
            val dateOrigin =
                SimpleDateFormat(dateInputFormat, locale ?: Locale.getDefault()).parse(dateInput)
            val dateResult = dateOrigin?.let {
                SimpleDateFormat(dateOutputFormat, locale ?: Locale.getDefault()).format(it)
            }
            if (ordinalSuffix == true && dateOrigin != null) {
                dateResult?.let {
                    return dateOrdinalSuffix(dateOrigin, it, locale ?: Locale.getDefault())
                }
            }
            return dateResult
        } catch (e: ParseException) {
    ...
    fun isSameHour(hour1: Int, hour2: Int): Boolean = hour1 == hour2
